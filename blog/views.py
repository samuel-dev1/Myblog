...
def BlogHome(request):
    try:
        ...
        context = {'posts':posts, 'postbanner':postbanner,"advert":advert}
    except:
        context={"null":None}
    ...
    try:
        if request.method == "POST":
            ...
            if searchtext is not None:
                posts =  UpdatePost.objects.all().order_by("-date_posted")
                advert = Advertisement.objects.all()
                product = UpdatePost.objects.filter(Q(title__icontains=searchtext)or Q(catgories_icontains=searchtext))
...
def ViewHome(request):
    try:
        posts =  UpdatePost.objects.all().order_by("-date_posted")
        advert = Advertisement.objects.all()
        return render(request,"pages/style-demo.html",{'posts':posts,"advert":advert})
    except:
...
def Entertaiment(request,element):
    try:
        ...
        advert = Advertisement.objects.all()
        return render(request,"pages/full-width.html",{"post":elements,'posts':posts,"elements":element,"advert":advert})
    except:
...
def details_view(request, slug):
    ...
    advert = Advertisement.objects.all()
    obj4  = UpdatePost.objects.filter(catgories = obj.catgories).order_by('-date_posted')
